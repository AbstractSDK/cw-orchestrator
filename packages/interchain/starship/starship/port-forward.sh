...
for i in $(seq 0 $num_chains); do
  ...
  sleep 1
  color yellow "chains: forwarded $chain lcd to http://localhost:$locallcd, rpc to http://localhost:$localrpc, grpc to http://localhost:$localfaucet, faucet to http://localhost:$localgrpc"
done
