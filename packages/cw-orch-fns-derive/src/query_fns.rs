...
pub fn query_fns_derive(input: ItemEnum) -> proc_macro2::TokenStream {
...
pub fn async_query_fns_derive(input: ItemEnum) -> proc_macro2::TokenStream {
    let name = &input.ident;
    let bname = Ident::new(&format!("Async{name}Fns"), name.span());
        ...
        match &mut variant.fields {
            Fields::Unnamed(variant_fields) => {
                ...
                quote!(
                    #variant_doc
                    #[allow(clippy::too_many_arguments)]
                    async fn #variant_func_name(&self, #(#variant_attr,)*) -> ::core::result::Result<#response, ::cw_orch::prelude::CwOrchError> {
                        let msg = #name::#variant_name (#(#variant_ident_content_names,)*);
                        self.async_query(&msg #maybe_into).await
                    }
                )
            }
            Fields::Unit => {
                quote!(
                    #variant_doc
                    async fn #variant_func_name(&self) -> ::core::result::Result<#response, ::cw_orch::prelude::CwOrchError> {
                        let msg = #name::#variant_name;
                        self.async_query(&msg #maybe_into).await
                    }
                )
    ...
    let derived_trait = quote!(
        /// Automatically derived trait that allows you to call the variants of the message directly without the need to construct the struct yourself.
        pub trait #bname<Chain: ::cw_orch::environment::AsyncWasmQuerier + ::cw_orch::environment::ChainState, #type_generics>: ::cw_orch::prelude::AsyncCwOrchQuery<Chain, QueryMsg = #entrypoint_msg_type #ty_generics > #where_clause {
            #(#variant_fns)*
        }
    );
    ...
    // We need to merge the where clauses (rust doesn't support 2 wheres)
    // If there is no where clause, we simply add the necessary where
    let necessary_where = quote!(SupportedContract: ::cw_orch::prelude::AsyncCwOrchQuery<Chain, QueryMsg = #entrypoint_msg_type #ty_generics >);
    let combined_where_clause = where_clause
        .map(|w| {
            quote!(
                #w #necessary_where
    ...
    let derived_trait_impl = quote!(
        #[automatically_derived]
        impl<SupportedContract, Chain: ::cw_orch::environment::AsyncWasmQuerier + ::cw_orch::environment::ChainState, #type_generics> #bname<Chain, #type_generics> for SupportedContract
        #combined_where_clause {}
    );
