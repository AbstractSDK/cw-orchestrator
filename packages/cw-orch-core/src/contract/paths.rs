...
mod artifacts_dir {
    use super::WasmPath;
    use crate::{
        build::BuildPostfix, env::EnvVar, environment::ChainState, error::CwEnvError,
        log::LOCAL_LOGS, CwOrchEnvVars,
    };
    ...
    impl ArtifactsDir {
        /// Get the artifacts directory from the environment variable `ARTIFACTS_DIR`.
        pub fn env() -> Self {
            let dir = CwOrchEnvVars::ArtifactsDir::parsed().unwrap_or_else(|_| {
                panic!(
                    "{} env variable not set",
                    CwOrchEnvVars::ArtifactsDir::ENV_VAR_NAME
                )
            });
            Self::new(dir)
        }
