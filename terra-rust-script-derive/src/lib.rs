...
use syn::token::Comma;
use syn::{self, parse_macro_input, DeriveInput, Field, Visibility};
    ...
    match data {
            ...
            for variant in &data_enum.variants {
                ...
                match &variant.fields {
                    ...
                    Fields::Named(fields_temp) => {
                        ...
                        let b: Punctuated<Field, Comma> = a
                            .iter()
                            .map(|field| {
                                let mut temp = field.clone();
                                temp.vis = Visibility::Inherited;
                                temp.attrs = vec![];
                                temp
                            })
                            .collect();
                        ...
                        let idents: Punctuated<Ident, Comma> =
                            a.iter().map(|ident| ident.ident.clone().unwrap()).collect();
        ...
        Data::Struct(data_struct) => {
            ...
            match &data_struct.fields {
                Fields::Named(fields_temp) => {
                    let is_variant_func_name = format_ident!("instantiate");
