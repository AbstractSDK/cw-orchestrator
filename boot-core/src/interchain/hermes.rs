...
impl Hermes {
    ...
    pub fn create_channel(
        ...
        let command = [
            ...
            channel_version,
            "--a-chain",
            &contract_a.get_chain().state.chain.chain_id,
            "--b-chain",
            &contract_b.get_chain().state.chain.chain_id,
            "--a-port",
            &port_a,
            "--b-port",
            &port_b,
        ...
        runtime.block_on(async {
            let create_exec_response = docker
                .create_exec(self.0.id.as_ref().unwrap(), create_exec_options)
                .await
                .expect("failed to create exec");

            let exec_id = create_exec_response.id;

            let StartExecResults::Attached { output, input: _ } = docker
