...
impl Coin {
    ...
    pub fn parse(str: &str) -> Result<Option<Coin>, BootError> {
        lazy_static! {
    ...
    ///
    pub fn parse_coins(str: &str) -> Result<Vec<Coin>, BootError> {
        let vec_res_opt_coins = str
            .split(',')
            .map(Coin::parse)
            .collect::<Vec<Result<Option<Coin>, BootError>>>();
        let mut coins: Vec<Coin> = Vec::with_capacity(vec_res_opt_coins.len());
        for vroc in vec_res_opt_coins {
            let coin_opt = vroc.map_err(|_source| BootError::CoinParseErrV {
                parse: str.parse().unwrap(),
            })?;

            match coin_opt {
                None => {
                    return Err(BootError::CoinParseErr(str.parse().unwrap()));
                }
                Some(coin) => {
                    coins.push(coin);
                }
