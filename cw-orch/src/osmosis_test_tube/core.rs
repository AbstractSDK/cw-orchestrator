...
/// Maps a RunnerError (from osmosis-test-tube), to a CwOrch Error
/// As we don't have a custom Error type for this integration, we use this helper function to map errors instead
pub fn map_err(e: RunnerError) -> CwOrchError {
    CwOrchError::StdErr(format!("Osmosis Test Tube Error : {}", e))
}
