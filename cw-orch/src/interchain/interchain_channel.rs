...
impl InterchainChannel{
	...
	pub fn get_chain(&self, chain_id: String) -> Result<InterchainPort, DaemonError>{
		if chain_id == self.port_a.chain_id{
			...
			return Err(DaemonError::ibc_err(format!("chain {}, doesn't exist in the InterchainChannel object {:?}", chain_id, self)))
		}
	}

	fn get_ordered_ports_from(&self, from: String) -> Result<(InterchainPort, InterchainPort), DaemonError>{
		if from == self.port_a.chain_id{
			...
			return Err(DaemonError::ibc_err(format!("chain {}, doesn't exist in the InterchainChannel object {:?}", from, self)))
		}
	}

	async fn get_tx_by_events_and_assert_one(channel: Channel, events: Vec<String>) -> Result<CosmTxResponse, DaemonError>{
	    let txs = Node::new(channel.clone()).find_some_tx_by_events(events, None, None).await?;
        if txs.len() != 1 {
	...
	pub async fn get_packet_send_tx(&self, from: String, packet_sequence: String) -> Result<CosmTxResponse, DaemonError>{
		...
		let send_events_string = vec![
            ...
            format!(
                ...
                dst_port.channel.clone().ok_or(DaemonError::ibc_err(format!("No channel registered between {:?} and {:?} on connection {}", self.port_a, self.port_b, self.connection_id)))?
            ),
	...
	pub async fn get_packet_receive_tx(&self, from: String, packet_sequence: String) -> Result<CosmTxResponse, DaemonError>{
	...
	pub async fn get_packet_ack_receive_tx(&self, from: String, packet_sequence: String) -> Result<CosmTxResponse, DaemonError>{
		let (src_port, dst_port) = self.get_ordered_ports_from(from)?;
	...
	pub async fn get_channel_creation_ack(&self, from: String) -> Result<Vec<CosmTxResponse>, DaemonError>{
		let (src_port, dst_port) = self.get_ordered_ports_from(from)?;

		let channel_creation_events_ack_events = vec![
            format!(
                "channel_open_ack.port_id='{}'",
                src_port.port
            ), // client is on chain1
            format!(
        ...
        // Here we just want to query all transactions with events, no other condition
       	Node::new(src_port.chain.clone()).find_tx_by_events(channel_creation_events_ack_events, None, Some(cosmrs::proto::cosmos::tx::v1beta1::OrderBy::Desc)).await
	}

	pub async fn get_channel_creation_confirm(&self, from: String) -> Result<Vec<CosmTxResponse>, DaemonError>{
		let (src_port, dst_port) = self.get_ordered_ports_from(from)?;

		let channel_creation_events_ack_events = vec![
            format!(
                "channel_open_confirm.port_id='{}'",
                dst_port.port
            ), 
            format!(
        ...
        // Here we just want to query all transactions with events, no other condition
       	Node::new(dst_port.chain.clone()).find_tx_by_events(channel_creation_events_ack_events, None, Some(cosmrs::proto::cosmos::tx::v1beta1::OrderBy::Desc)).await
	}
	...
	pub async fn get_last_channel_creation(&self, from: String) -> Result<(Option<CosmTxResponse>, Option<CosmTxResponse>), DaemonError>{

        let current_channel_creation_a = self.get_channel_creation_ack(from.clone()).await?
            .get(0).cloned();

        let current_channel_creation_b = self.get_channel_creation_confirm(from).await?
            .get(0).cloned();
	...
	// We get the last transactions that is related to creating a channel from chain from to the counterparty chain defined in the structure
	pub async fn get_last_channel_creation_hash(&self, from: String) -> Result<(Option<String>, Option<String>), DaemonError>{

		let (current_channel_creation_a, current_channel_creation_b) = self.get_last_channel_creation(from).await?;
		Ok((current_channel_creation_a.map(|tx| tx.txhash), current_channel_creation_b.map(|tx| tx.txhash)))
	}
	            ...
	            Ok(tx) => {
	                if let Some(ack_tx) = tx.0 {
	                	if let Some(confirm_tx) = tx.1{
		                    if 
		                    	ack_tx.txhash != last_chain_creation_hashes.0.clone().unwrap_or("".to_string()) 
		                    		&&
		                    	confirm_tx.txhash != last_chain_creation_hashes.1.clone().unwrap_or("".to_string()) {
	...
	pub async fn follow_packet(&self, from: String, sequence: String) -> Result<Vec<TxId>, DaemonError>{
	    ...
	    let recv_packet_sequence =
	        received_tx.get_events("write_acknowledgement")[0] // There is only one acknowledgement per transaction possible
	    ...
	    let decoded_ack: String = if let Ok(ack_result) = parsed_ack {
	        match ack_result {
	            AckResponse::Result(b) => {
	            	match  std::str::from_utf8(&general_purpose::STANDARD.decode(b.clone()).unwrap_or(vec![])){
	            		Ok(d) => format!(
		                    "Decoded successful ack : {}",
		                    d
		                ),
	            		Err(_) => format!(
		                    "Couldn't decode following successful ack : {}",
		                    b
		                ),
	            	}
	            }
	    ...
	    let ack_tx = self.get_packet_ack_receive_tx(src_port.chain_id.clone(), sequence.clone()).await?;
