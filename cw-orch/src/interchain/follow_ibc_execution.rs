...
pub async fn follow_trail(
    ...
    let counter_party_grpc_channels: Vec<Channel> = join_all(chain_ids.iter().map(|chain| async {
        get_channel(chain.clone(), configure_local_network)
            .await
            .unwrap()
    }))
    .await;
