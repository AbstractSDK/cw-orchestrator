...
use cw_orch_daemon::queriers::{Ibc, Staking};
...
pub fn main() -> AnyResult<()> {
    ...
    let balance_result = runtime.block_on(bank_query_client.balance(sender, None))?;
    println!("Balance of {} : {:?}", sender, balance_result);

    // We do an actual Staking query on MAINNET
    let staking_query_client: Staking = daemon.query_client();
    let validator = "junovaloper185hgkqs8q8ysnc8cvkgd8j2knnq2m0ah6ae73gntv9ampgwpmrxqlfzywn";
    let validator_result = runtime.block_on(staking_query_client.validator(validator))?;
    println!("Validator info of {} : {:?}", sender, validator_result);

    // We do an actual IBC query on MAINNET
    let ibc_query_client: Ibc = daemon.query_client();
    let port_id = "transfer";
    let channel_id = "channel-0";
    let channel_result = runtime.block_on(ibc_query_client.channel(port_id, channel_id))?;
    println!(
